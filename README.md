...
The above example illusrates that the two strokes are combined to form a single symbole, 'x'. One of the challenges in the task is that the symbol recognition requires the stroke recognition and stroke recognition, in turn, requires the symbol recognition. This is partially due to the confusions in the stroke recognition (e.g., “C and (“, “9 and q”). We human process two steps (symbol and stroke levels) in parallel without conscious efforts because we process them in a very fast and subconscious way(after certain level of training and learning). Unfortunately, computers need to learn these steps. For each symbol, the distribution of the number of strokes were examined. The following shows an example for symbol 4, 5, and 6. 
